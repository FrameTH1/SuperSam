...
<body>
    ...
    <div class="shadow card p-3" 
         onclick='openModal({
             ...
             types: JSON.parse("${row.types.replace(/'/g, "\\'").replace(/"/g, '\\"')}"),
             status: "${row.status}",
        ...
        <div class="px-1 d-flex">
            ...
            function openModal(row) {
                document.getElementById('modal-title').innerText = row.title;
                document.getElementById('modal-img').src = row.img;
                document.getElementById('modal-fname').innerText = 'ชื่อ : ' + row.fname;
                document.getElementById('modal-rating').innerText = 'คะแนน :'
                document.getElementById('modal-stars').style.setProperty('--rating', checkValue2(row.rating));
                document.getElementById('modal-rating-count').innerText = `( ${row.rating_count} โหวต )`;
                document.getElementById('modal-contact').innerText = 'ข้อมูลติดต่อ : ' + checkValue(row.contact);
                document.getElementById('modal-type').innerText = 'หมวดหมู่งาน : ' + checkValue(row.types);
                document.getElementById('modal-description').innerText = 'รายละเอียดงาน : ' + checkValue(row.description);
                document.getElementById('modal-price').innerText = 'ราคา : ' + row.price + ' บาท';

                const sessionUserId = <?php echo json_encode($_SESSION["userId"]); ?>;
                const sessionDisplayName = <?php echo json_encode($_SESSION["displayName"]); ?>;
                const contactButton = document.querySelector('.btn-contact');
                const confirmButton = document.querySelector('.btn-confirm-accept');

                if ((row.status === 'รอคนหางาน' || row.status === 'รอคนจ้างงาน')) {
                ...
                contactButton.onclick = function () {
                    if (row.status === 'รอคนหางาน') {
                        window.location.href = '/?page=find&job=' + row.id + '&id=' + row.receiver_id;
                    } else if (row.status === 'กำลังดำเนินการของหางาน' || row.status === 'การดำเนินการหางานเสร็จสิ้น') {
                        window.location.href = '/?page=find&job=' + row.id + '&id=' + row.receiver_id;
                    } else if (row.status === 'กำลังดำเนินการของจ้างงาน' || row.status === 'การดำเนินการจ้างงานเสร็จสิ้น') {
                        window.location.href = '/?page=hire&job=' + row.id + '&id=' + row.receiver_id;
                    } else {
            ...
            function checkValue(input) {
                return input === '' || input === 'null' || input.length === 0 ? 'ไม่ได้ระบุ' : input;
            }
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="d-flex gap-1 mb-2">
                            <p class="h6 my-auto" id="modal-rating"></p>
                            <div class="d-flex">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary btn-contact" data-bs-toggle="modal"
                            data-bs-dismiss="modal">ติดต่อ</button>
                        <button type="button" class="btn btn-success btn-confirm-accept" data-bs-toggle="modal"
                            data-bs-target="#confirmModal" data-bs-dismiss="modal">ขั้นตอนต่อไป</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="modal fade" id="ratingModal" tabindex="-1" aria-labelledby="ratingModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="ratingModalLabel">ให้คะแนน</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-success btn-confirm-rating" data-bs-toggle="modal"
                            data-bs-target="#confirmModal" data-bs-dismiss="modal">ขั้นตอนต่อไป</button>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        document.querySelector('.btn-confirm-rating').addEventListener('click', function () {
            // ค้นหาค่า rating ที่ถูกเลือก
            ...
            if (selectedRating) {
                ...
                // ส่งค่า rating ไปยังเซิร์ฟเวอร์
                fetch('action/set_rating.php', {
                    method: 'POST',
                    headers: {
                    ...
                    body: new URLSearchParams(
                        {
                            jobId: idPost, // ID ของงานที่ต้องการอัปเดต rating
                            rating: ratingValue
                        }
                    )
                })
                    ...
                    .then(data => {
                        if (data.success) {
                            console.log("Rating ได้รับการอัปเดตเรียบร้อยแล้ว:", ratingValue);
                        } else {
                            console.error("เกิดข้อผิดพลาดในการอัปเดต rating:", data.message);
                        }
                    })
                    .catch(error => {
                        console.error('Error:', error);
                ...
                console.log("ยังไม่ได้เลือก rating");
            }
        });
        ...
        document.querySelector('.btn-confirm-accept').addEventListener('click', function () {
            ...
            if (selectedRating) {
                console.log("Rating ที่เลือกคือ:", selectedRating.value);
            } else {
                console.log("ยังไม่ได้เลือก rating");
            }
        });
        ...
        document.querySelector('.btn-confirm-accept').addEventListener('click', function () {
            ...
            fetch('action/set_post_status.php', {
                ...
                .then(data => {
                    if (data.success) {
                        // ทำสิ่งที่คุณต้องการเมื่ออัพเดทสำเร็จ
                        // ตัวอย่าง: ปิด Modal
                        $('#confirmModal').modal('hide');  // ใช้ jQuery เพื่อปิด modal
                        if (data.rating) {
                            $('#ratingModal').modal('show');  // ใช้ jQuery เพื่อปิด modal
                        }
                    } else {
